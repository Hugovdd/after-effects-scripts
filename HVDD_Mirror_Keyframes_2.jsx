    ...
    if (app.project.activeItem instanceof CompItem) { // Check if a composition is open
        ...
        if (comp && comp.selectedProperties.length > 0) {// Check if properties are selected
            ...
            for (var k = 0; k < theKeys.keys.length; k++){ //Loop through array of keys
                ...
                var newKeytime = theKeys.lastSelectedKeyTime - keyTime + comp.time; // Offset keyTime by CTI
                if (comp.time < theKeys.firstSelectedKeyTime){ // If CTI before keyframes offset them differently
                    var difference = theKeys.lastSelectedKeyTime - theKeys.firstSelectedKeyTime;
                    newKeytime -= difference;
                } 
