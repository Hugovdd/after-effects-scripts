...
function applyTextEvo() {
  app.beginUndoGroup("Apply TextEvo Binance");
  ...
  function applyAnimatorProps(layer){
    ...
    if (prop.property("TextEvo Binance") == null){
        ...
        layer.property("ADBE Text Properties").property(4).property(1).property(1).property(1).name = "textevo-binance";
  ...
  function getTextEvo(layer , selection ,textAnimator){
    if(selection == null) selection = true;
    if(textAnimator == null) textAnimator = true;
    var textevoArray = [];
    comp= DuAEF.DuAE.Project.getActiveComp();
    if(comp == null) return null;

    if(comp.selectedProperties.length > 0 && selection == true){
        for(var i = 0 ; i < comp.selectedProperties.length ; i++){
            var currentFx = comp.selectedProperties[i];
            if(currentFx.matchName == "Pseudo/textevo"){
                
                if(DuAEF.DuJS.Array.indexOf(textevoArray , currentFx.name) == -1){
                    textevoArray.push(currentFx.name);
                }
            }else if (currentFx.matchName == "ADBE Text Animator" && textAnimator == true){
                if(DuAEF.DuJS.Array.indexOf(textevoArray , currentFx.name)== -1){
                    textevoArray.push(currentFx.name);
                }
            }
        ...
        for(var i = 0 ; i < layer("ADBE Effect Parade").numProperties ; i++){
            var currentFx = layer("ADBE Effect Parade")(i+1);
            if(currentFx.matchName == "Pseudo/textevo"){
                textevoArray.push(currentFx.name);
            }
        }
  ...
  if (!thisComp || !(thisComp instanceof CompItem)){
    ...
    for (i = 0; i < selLayers.length; i++) { // Loop through selected Layer
      removeTextEvo(selLayers[i]);
      applyPseudoEffect(selLayers[i]);  
      applyAnimatorProps(selLayers[i]);
      applyExpressions(selLayers[i]);
